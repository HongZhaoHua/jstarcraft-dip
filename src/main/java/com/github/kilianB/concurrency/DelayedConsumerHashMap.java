...
/**
 * Accepts a consumer which executes all provided tasks in a batch no more often
 * than the delay specified. Tasks are labeled by id and will replace each other
 * if they arrive within the grace period while ensuring that the task scheduled
 * last will eventually be executed.
 * 
 * <ol>
 * <li>Event( key = 1) arrives. Consumer has not executed a task therefore it is
 ...
 * <li>Event( key = 2) arrives. Consumer is currently sleeping. The first event
 * with key 2 is replaced and will never be executed.
